...
% this way, i can check that my tidying up is just that
rng('default');
rng(20160526); 
...
if load_flag==0
   ...
   %for information about the make-up of the population structure and the innovation_record, look at initial_population_ORI.m
   [population,innovation_record]=initial_population_ORI(population_size, number_input_nodes, number_output_nodes, vector_connected_input_nodes);
...
while generation<maxgeneration & flag_solution==0
   ...
   population=xor_experiment_vec_ORI(population);
   ...
   [population,species_record,innovation_record]=reproduce_ORI(population, species_record, innovation_record, initial, selection, crossover, mutation, speciation, generation, population_size);
   toc;
   end
