...
function [newPop,newSpecLog,newInnovLog]=reproduce(pop, specLog, innovLog, initial, selection, crossover, mutation, speciation, generation, population_size)
...
% 3.erase lowest percentage (initial.killRate) in species with more than initial.number_for_kill individuals to keep them from taking part in reproduction
...
for index_species=1:size(specLog,2)
   sum_average_fitnesses=sum_average_fitnesses+specLog(index_species).generation_record(2,size(specLog(index_species).generation_record,2))*(specLog(index_species).number_individuals>0);
end
...
for index_species=1:size(specLog,2)
   if specLog(index_species).number_individuals>0 %test if species existed in old generation
      number_offspring=specLog(index_species).generation_record(2,size(specLog(index_species).generation_record,2))/sum_average_fitnesses*population_size; %compute number of offspring in new generation
      ...
      if number_offspring>0 %Check to see if species is dying out, only add those species to matrix_existing_and_propagating_species which have offspring in the new generation
         matrix_existing_and_propagating_species=[matrix_existing_and_propagating_species,[specLog(index_species).ID;number_offspring;0]]; %matrix (objective 1)
         if specLog(index_species).number_individuals>=initial.number_copy %check for condition for objective 2
            index_individual=index_individual+1;
            newPop(index_individual)=pop(specLog(index_species).generation_record(4,size(specLog(index_species).generation_record,2))); % Objective 2
      ...
      if specLog(index_species).number_individuals>initial.killRate && ceil(specLog(index_species).number_individuals*(1-initial.killRate))>2 %check condition for objective 3
         matrix_individuals_species=[find([pop(:).species]==index_species);[pop([pop(:).species]==index_species).fitness]];
         ...
         for index_kill=1:floor(specLog(index_species).number_individuals*initial.killRate)
            pop(sorting_vector(index_kill)).species=0; %objective 3
         end         
      end
...
index_ref=0;
for index_species_ref=1:size(specLog,2)
   if sum([pop(:).species]==index_species_ref)>0 %Check if species exists in old population 
      index_ref=index_ref+1;
      [~,index_ref_old]=max(([pop(:).species]==index_species_ref).*rand(1,size(pop,2)));
      population_ref(index_ref)=pop(index_ref_old);
   end
end 
...
for index_species=1:size(matrix_existing_and_propagating_species,2)
   ...
   fitnesses_species=[pop(find([pop(:).species]==species_ID)).fitness];
   index_fitnesses_species=find([pop(:).species]==species_ID);
   [~,sorted_fitnesses]=sort(fitnesses_species);
   ...
   [NewChrIx, ~] = find(Mt < Mf & [ zeros(1, Nsel); Mf(1:Nind-1, :) ] <= Mt); % second output argument is needed, changes first!
   ...
   while matrix_existing_and_propagating_species(3,index_species)<matrix_existing_and_propagating_species(2,index_species) %cycle until actual number of offspring has reached allotted number of offspring                                                          
      ...
      if count_individuals_species<=number_crossover %O.k: we are doing crossover
         %Select Parents
         %select parent1          
         parent1=pop(NewChrIx(2*count_individuals_species-1));
         %select parent2
         found_parent2=0;
         ...
         if rand<crossover.probability_interspecies && size(matrix_existing_and_propagating_species,2)>1 %select parent2 from other species (can only be done if there is more than one species in old population)          
            while found_parent2==0;                
               [~,index_parent2]=max(rand(1,size(pop,2)));
               parent2=pop(index_parent2);
            ...
            parent2.fitness=parent1.fitness; %set fitnesses to same to ensure that disjoint and excess genes are inherited fully from both parents (tip from ken)
             ...
             parent2=pop(NewChrIx(2*count_individuals_species));
         end
         ...
         new_individual=pop(NewChrIx(number_crossover+count_individuals_species));
      end
      ...
      vector_possible_connect_to_nodes=new_individual.nodegenes(1,new_individual.nodegenes(2,:)==2 | new_individual.nodegenes(2,:)==3); %connections can only run into hidden and output nodes
      ...
      if rand<mutation.probability_add_connection && number_possible_connection>0 && flag1==0 %check if new connections can be added to genes (if there are any possible connections which are not already existing in genes of new individual)
         ...
         while flag_connection_ok==0 && index_new_connection<size(new_connection_matrix,2) 
            ...
            while flag_recurrent==0 && depth<size(new_individual.connectiongenes,2) && ~isempty(nodes_current_level)
               ...
               for index_check=1:size(nodes_current_level);                  
                  nodes_next_level=[nodes_next_level new_individual.connectiongenes(3,new_individual.connectiongenes(2,:)==nodes_current_level(index_check))];
               end
         ...
         if flag_connection_ok
            index_already_happened=find((innovLog(5,:)==generation).*(innovLog(2,:)==new_connection(1)).*(innovLog(3,:)==new_connection(2))); %set flag signifying new innovation (connection not contained in innovation_record of this generation)  
            new_innovation=not(sum(index_already_happened)); 
            if new_innovation==1 % O.K. is new innovation
               new_connection=[max(innovLog(1,:))+1;new_connection]; %Update the new connection with its innovation number
               ...
               innovLog=[innovLog,[new_connection;0;generation]];               
            else % connection gene already exists in innovation_record of this generation
               % Update connection_genes
               new_individual.connectiongenes=[new_individual.connectiongenes,[innovLog(1:3,index_already_happened);rand*2-1;1]];         
            end
         end         
      end
      ...
      new_innovation=0;
      if flag1==1
         max_old_innovation_number=max((innovLog(5,:)<generation).*innovLog(1,:)); %highest innovation number from last generation (to ensure that only connections from from last generation or older are chosen for add node mutation, otherwise a new connection added in the last mutation might instantly be disabled)
         ...
         if sum(exist_innovation)>0 %if these are fulfilled, we have to test for connect_to node to see if innovation really is the same
            for index_check=1:length(exist_innovation)
               if innovLog(3,exist_innovation(index_check)+1)==insert_node_connection(2)
                  new_innovation=0;                   
         ...
         if new_innovation==1 %O.K. is true innovation for current generation
            % Update node_genes
            new_node_number=max(innovLog(4,:))+1;
            ...
            new_connections=[[max(innovLog(1,:))+1;insert_node_connection(1);new_node_number;1;1],[max(innovLog(1,:))+2;new_node_number;insert_node_connection(2);new_individual.connectiongenes(4,insert_node_connection(3));1]];
            ...
            % Update innovation_record
            innovLog=[innovLog,[new_connections(1:3,:);new_node_number,0;generation,generation]];
         else %no new innovation, has already happened at least once in this generation
            % Update node_genes
            node_number=innovLog(4,index_already_existent_this_generation);
            new_individual.nodegenes=[new_individual.nodegenes,[node_number;3;0;0]];
            % Update connection_genes
            new_individual.connectiongenes(5,insert_node_connection(3))=0; %disable old connection gene
            new_connections=[innovLog(1:3,index_already_existent_this_generation:index_already_existent_this_generation+1);1,new_individual.connectiongenes(4,insert_node_connection(3));1,1];        
      ...
      while species_assigned==0 && index_population_ref<size(population_ref,2)
         ...
         %WEIRD, max_num_genes is not used... look into original C-code
         %implementation to see what it does there ...
...
for index_species=1:size(specLog,2)
   specLog(index_species).number_individuals=sum([newPop(:).species]==index_species);
end
%assign updated species_record to output
newSpecLog=specLog;
%assign updated innovation_record to output
newInnovLog=innovLog;